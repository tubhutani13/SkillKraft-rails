class RecommendationService
  ...
  def self.calculate_recommendations(mentee)
    ...
    mentors = User.where.not(id: mentee.id).includes(:expert_skills, :learning_skills, [profile_picture_attachment: :blob]).joins(expert_skills: :expert_users).where(expert_skills: { id: mentee_skill_ids })
                  .distinct
